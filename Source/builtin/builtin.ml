...
let quot a b = let q = a / b in
       if q < 0 && (a mod b != 0) then q - 1 else q;;
 ...
 *)
let modulo a b = if sign b = -1 then failwith "b has to be a natural number"
...
*)
let div a b = if sign b = -1 then failwith "b has to be a natural number"
